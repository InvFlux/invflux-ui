...
export function ProPill(props: ProPillProps): JSX.Element {
  return (
    <span class={`inline-flex items-center gap-1 rounded bg-yellow-100 px-1.5 py-0.5 text-xs font-medium text-yellow-800 ring-1 ring-yellow-200${props.class ? ` ${props.class}` : ''}`}>
      {/* eslint-disable-next-line solid/no-innerhtml -- module-const SVG literal in this file, not user or server data. */}
      <span class="inline-flex [&_svg]:h-3.5 [&_svg]:w-auto" innerHTML={INVFLUX_MARK} aria-hidden="true" />
      {props.tier ?? 'Pro'}
    </span>
  );
}
