...
describe('diffTokens (words)', () => {
  it('marks a replaced word as del + add and keeps the rest equal', () => {
    ...
    const joinedDel = segs
      .filter((s) => s.kind !== 'add')
      .map((s) => s.value)
      .join('');
    const joinedAdd = segs
      .filter((s) => s.kind !== 'del')
      .map((s) => s.value)
      .join('');
  ...
  it('retains a common prefix when punctuation is appended to a word', () => {
    const segs = diffTokens(
      "Let's try adding a note here",
      "Let's try adding a note here... or there",
      'words',
    );
    const dels = segs
      .filter((s) => s.kind === 'del')
...
describe('thread helpers', () => {
  it('latestLiveVersion returns the newest non-deleted version', () => {
    const t = thread([
      version({ version: 1, body: 'first' }),
      version({ version: 2, action: 'edited', body: 'second' }),
    ]);
