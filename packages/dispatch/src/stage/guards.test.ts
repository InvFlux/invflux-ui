...
describe('checkStageGuard', () => {
  it('ok — scanner input on EAN product by non-admin', () => {
    expect(
      checkStageGuard({ shippableQty: 1, hasEan: true, isAdmin: false, inputSource: 'scanner' }),
    ).toBe('ok');
  });

  it('ean-noscan — keyboard input on EAN product by non-admin', () => {
    expect(
      checkStageGuard({ shippableQty: 1, hasEan: true, isAdmin: false, inputSource: 'keyboard' }),
    ).toBe('ean-noscan');
  });

  it('ok — keyboard input on EAN product by admin', () => {
    expect(
      checkStageGuard({ shippableQty: 1, hasEan: true, isAdmin: true, inputSource: 'keyboard' }),
    ).toBe('ok');
  });

  it('ok — keyboard input on no-EAN product by non-admin', () => {
    expect(
      checkStageGuard({ shippableQty: 1, hasEan: false, isAdmin: false, inputSource: 'keyboard' }),
    ).toBe('ok');
  });

  it('no-shippable-qty — takes priority over EAN enforcement', () => {
    expect(
      checkStageGuard({ shippableQty: 0, hasEan: true, isAdmin: false, inputSource: 'keyboard' }),
    ).toBe('no-shippable-qty');
  });

  it('no-shippable-qty — even with scanner input', () => {
    expect(
      checkStageGuard({ shippableQty: 0, hasEan: false, isAdmin: true, inputSource: 'scanner' }),
    ).toBe('no-shippable-qty');
  });
});
