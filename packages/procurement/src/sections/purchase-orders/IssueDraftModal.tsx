...
export function IssueDraftModal(props: IssueDraftModalProps): JSX.Element {
  ...
  const totalQty = createMemo(() => kept().reduce((s, l) => s + l.requestedQty, 0));
  const totalValue = createMemo(() => kept().reduce((s, l) => s + l.requestedQty * (effectiveCost(l) ?? 0), 0));

  // MOQ / case-pack violations among the KEPT lines (a removed 0-qty line is not "below MOQ").
  const belowMoqLines = createMemo(() => kept().filter((l) => belowMoq(l.requestedQty, props.moqFor(l.subjectId))));
  const offPackLines = createMemo(() => kept().filter((l) => offCasePack(l.requestedQty, props.casePackFor(l.subjectId))));
  const invalidQtyLines = createMemo(() => kept().filter((l) => belowMoq(l.requestedQty, props.moqFor(l.subjectId)) || offCasePack(l.requestedQty, props.casePackFor(l.subjectId))));

  // Zero-qty removed lines that carry a usable replenishment suggestion — rescuable by [Set suggested].
  const fixableZeroQty = createMemo(() => removed().filter((r) => 'zero_qty' === r.reason && null !== r.line.suggestedQty && r.line.suggestedQty > 0).map((r) => r.line));

  const fixAllQty = (): void =>
    props.onFixQtys(invalidQtyLines().map((l) => ({ id: l.id, qty: nextValidQty(l.requestedQty, props.moqFor(l.subjectId), props.casePackFor(l.subjectId)) })));
  const fixZeroQty = (): void => props.onFixQtys(fixableZeroQty().map((l) => ({ id: l.id, qty: l.suggestedQty as number })));
  ...
  const confirmLabel = (): string =>
    removed().length > 0
      ? sprintf(_n('Remove %d & download', 'Remove %d & download', removed().length), removed().length)
      : __('Assign number & download');

  const linkBtn = 'rounded border border-primary px-2 py-1 text-xs font-medium text-primary hover:bg-primary/5 disabled:opacity-50';

  return (
    <Modal onClose={props.onCancel} closeOnBackdrop={false} backdropClass="flex items-center justify-center bg-black/30 p-6" label={__('Assign a purchase order number')}>
      <div class="flex w-full max-w-lg flex-col overflow-hidden rounded-lg border border-border bg-surface shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div class="border-b border-border bg-surface-raised p-4">
          <h2 class="text-lg font-semibold text-text">{__('Assign a number and download?')}</h2>
        </div>

        <div class="space-y-3 p-4 text-sm text-text">
          <p class="">
            {sprintf(__('This order gets its purchase order number, ready to send to %s.'), props.supplierLabel)}{' '}
            {__('The number is permanent, so your copy and theirs always match. You can still edit the lines until you mark it as sent.')}
          </p>
          ...
          <div class="flex flex-wrap gap-x-6 gap-y-1 rounded border border-border bg-surface-raised px-3 py-2 tabular-nums">
            <span>
              <span class="font-semibold">{kept().length}</span> <span class="text-text-muted">{_n('SKU', 'SKUs', kept().length)}</span>
            </span>
            <span>
              <span class="text-text-muted">{__('Total qty')}:</span> <span class="font-semibold">{totalQty()}</span>
            </span>
            <span>
              <span class="text-text-muted">{__('Total value')}:</span> <span class="font-semibold">{money(totalValue())}</span>
            </span>
          </div>
          ...
          <Show when={invalidQtyLines().length > 0}>
            <div class="space-y-1.5 rounded border border-amber-300 bg-amber-50 px-3 py-2 text-amber-800">
              <Show when={belowMoqLines().length > 0}>
                <p>⚠️ {sprintf(_n('%d SKU has a quantity below its minimum order quantity.', '%d SKUs have a quantity below their minimum order quantity.', belowMoqLines().length), belowMoqLines().length)}</p>
              </Show>
              <Show when={offPackLines().length > 0}>
                <p>⚠️ {sprintf(_n('%d SKU has a quantity that is not a whole case pack.', '%d SKUs have a quantity that is not a whole case pack.', offPackLines().length), offPackLines().length)}</p>
              </Show>
          ...
          <Show when={removed().length > 0}>
            <div class="space-y-1.5 rounded border border-red-200 bg-red-50 px-3 py-2 text-red-700">
              <p>
                {sprintf(_n('%d line will be removed from the order:', '%d lines will be removed from the order:', removed().length), removed().length)}
              </p>
              <ul class="max-h-40 list-disc space-y-0.5 overflow-y-auto pl-5">
                <For each={removed()}>
                  {(r) => (
                    <li>
                      {r.line.productLabel || `#${r.line.id}`} <span class="text-red-400">— {reasonLabel(r.reason)}</span>
                    </li>
                  )}
                </For>
              </ul>
              ...
              <Show when={fixableZeroQty().length > 0}>
                <div class="flex items-center gap-2 pt-0.5">
                  <span>⚠️ {sprintf(_n('%d has a suggested quantity.', '%d have a suggested quantity.', fixableZeroQty().length), fixableZeroQty().length)}</span>
                  <button type="button" class={linkBtn} disabled={props.fixing} onClick={fixZeroQty}>
        ...
        <div class="flex justify-end gap-2 border-t border-border bg-surface-raised p-4">
          <button type="button" class="rounded border border-border bg-surface px-3 py-1.5 text-sm font-medium text-text shadow-sm hover:bg-gray-100" onClick={props.onCancel}>
            {__('Cancel')}
          </button>
          <button type="button" class="rounded bg-amber-600 px-3 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-amber-700 disabled:opacity-60" disabled={props.issuing || props.fixing || 0 === kept().length} onClick={props.onConfirm}>
            {confirmLabel()}
          </button>
        </div>
      </div>
