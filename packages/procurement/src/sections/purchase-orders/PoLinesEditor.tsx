...
export function PoLinesEditor(props: EditorProps): JSX.Element {
  ...
  const casePackFor = (subjectId: number): number | null => catalogueItem(subjectId)?.casePack ?? null;
  // Catalogue unit price — the value a null (inherited) line cost displays faded and freezes to at submit.
  const catalogPriceFor = (subjectId: number): string | null => catalogueItem(subjectId)?.unitPrice ?? null;
  const addOptions = (): AddOption[] => {
    const onPo = new Set(props.lines.map((l) => l.subjectId));
    return (catalogue.data?.products ?? [])
      .filter((p) => !onPo.has(p.subjectId))
      .map((p) => ({ subjectId: p.subjectId, label: null === p.sku ? p.productLabel : `${p.productLabel} · ${p.sku}`, unitPrice: p.unitPrice }));
  };

  const setLines = (fn: (lines: PoLine[]) => PoLine[]): void => {
    queryClient.setQueryData<PurchaseOrderDetail>(key(), (old) => (old ? { ...old, lines: fn(old.lines) } : old));
  };
  ...
  let tempId = -1;
  const buildOptimisticLine = (subjectId: number, label: string, qty: number, unitCost: string | null): PoLine => {
  ...
  const addDraftLine = async (subjectId: number): Promise<number | null> => {
    try {
      const { line } = await api.post<{ line: PoLine }>(`/procurement/purchase-orders/${props.poId}/lines`, {
  ...
  const delMany = createMutation(() => ({
    mutationFn: (ids: number[]) => Promise.all(ids.map((id) => api.del(`/procurement/purchase-orders/${props.poId}/lines/${id}`))),
  ...
  const grandTotal = (): number => props.lines.reduce((s, l) => s + (null === l.lineTotal ? 0 : Number(l.lineTotal)), 0);

  // Apply an immediate-commit patch to a line's optimistic cache row (mirrors the server's update).
  const applyPatch = (l: PoLine, patch: DraftLinePatch): PoLine => {
    const next: PoLine = { ...l };
    if (undefined !== patch.requested_qty) next.requestedQty = patch.requested_qty;
    if ('unit_cost' in patch) next.unitCost = patch.unit_cost ?? null;
    if ('note' in patch) next.note = patch.note ?? null;
    next.lineTotal = null === next.unitCost ? null : (next.requestedQty * Number(next.unitCost)).toFixed(4);
    return next;
  };
  ...
  const editLine = createMutation(() => ({
    ...
    onError: (_e: unknown, _v: { id: number; patch: DraftLinePatch }, ctx: { prev?: PurchaseOrderDetail } | undefined) => {
      if (ctx?.prev) queryClient.setQueryData(key(), ctx.prev);
  ...
  const fmtCost = (v: string): string => ('' === v || '-' === v || Number.isNaN(Number(v)) ? v : Number(v).toFixed(props.costDecimals));
  const importFields: ImportField[] = [
    ...
    { key: 'requested_qty', label: __('Quantity'), aliasKey: 'qty', aliases: [...COMMON_ALIASES.qty, 'ordered', 'order qty', 'quantité commandée'], numeric: true },
    { key: 'unit_cost', label: __('Unit cost'), aliasKey: 'cost', aliases: [...COMMON_ALIASES.cost], numeric: true, format: fmtCost },
  ];
  ...
  const resolveImport = async (rows: MappedRow[], keyField: string): Promise<ResolvedRow[]> => {
    ...
    return rows.map((r): ResolvedRow => {
      ...
      //   value      → set it.
      const qtyCell = 'requested_qty' in r ? (r.requested_qty ?? '').trim() : undefined;
      ...
      const data: Record<string, unknown> =
        null !== hit.postId ? { post_id: hit.postId, is_variation: hit.isVariation } : { subject_id: hit.subjectId };
      if (undefined !== qtyCell && '' !== qtyCell) data.requested_qty = '-' === qtyCell ? 0 : Number(qtyCell) || 0;
      if (undefined !== costCell && '' !== costCell) data.unit_cost = '-' === costCell ? null : costCell;
      ...
      const supSkuCell = 'supplier_sku' in r && 'supplier_sku' !== keyField ? (r.supplier_sku ?? '').trim() : undefined;
      if (undefined !== supSkuCell && '' !== supSkuCell) data.supplier_sku = '-' === supSkuCell ? '' : supSkuCell;
      ...
      if (existingLine) {
        if ('requested_qty' in r) existing.requested_qty = String(existingLine.requestedQty);
        if ('unit_cost' in r) existing.unit_cost = null === existingLine.unitCost ? '' : String(existingLine.unitCost);
      }
  ...
  const commitImport = async (matched: ResolvedRow[]): Promise<void> => {
    const { added, updated } = await api.post<{ added: number; updated: number }>(`/procurement/purchase-orders/${props.poId}/lines/bulk`, {
    ...
    <div>
      <div class="mb-2 flex items-center gap-3">
        <div class="relative">
          ...
          <Show when={adding()}>
            <AddPicker options={addOptions()} onAdd={(o) => add.mutate(o)} onClose={() => setAdding(false)} />
          </Show>
        </div>
        <Button
          variant="secondary"
