...
/** The supplier documents whose content this Essentials flow captures onto the PO (the document itself is Pro). */
type DocType = 'oa' | 'asn';
 ...
 */
export function PoExpectedCaptureGrid(props: { poId: number; lines: PoLine[]; onDone: () => void }): JSX.Element {
  const ctx = useProcurement();
  const api = createApi(ctx);
  const queryClient = useQueryClient();

  const [docType, setDocType] = createSignal<DocType>('oa');
  ...
  const expectedFor = (line: PoLine): number | null => {
    ...
    return undefined === e ? line.expectedQty : e;
  };
  const editedLines = (): Array<{ poLineId: number; expectedQty: number | null }> =>
    Object.entries(edits()).map(([id, v]) => ({ poLineId: Number(id), expectedQty: v }));
  ...
  const importFields: ImportField[] = [
    ...
    { key: 'supplier_sku', label: __('Supplier SKU'), aliases: [...COMMON_ALIASES.supplier_sku], keyCandidate: true, keyPriority: 2, keyOnly: true },
    { key: 'gtin', label: __('Barcode (GTIN/EAN)'), aliases: [...COMMON_ALIASES.gtin], keyCandidate: true, keyPriority: 3, keyOnly: true },
    {
      ...
      aliasKey: 'expected',
      aliases: ['confirmed', 'confirmed qty', 'confirmed quantity', 'confirmé', 'confirmée', 'quantité confirmée', 'qté confirmée', 'prévu', 'quantité prévue', 'expected qty', 'expected quantity', 'oa qty', 'asn qty'],
      numeric: true,
    },
  ];
  ...
  const lineIndex = (keyField: string): Map<string, PoLine> => {
    const pick = (l: PoLine): string | null => ('supplier_sku' === keyField ? l.supplierSku : 'gtin' === keyField ? l.gtin : l.sku);
  ...
  const resolveImport = async (rows: MappedRow[], keyField: string): Promise<ResolvedRow[]> => {
    ...
    return rows.map((r): ResolvedRow => {
      ...
      if (!line) return { key: k, label: k, status: 'unmatched', note: __('No matching line on this PO') };
      // Per-cell intent: empty → leave untouched; '-' → clear to unknown; value → set.
      const cell = 'expected_qty' in r ? (r.expected_qty ?? '').trim() : '';
      const expectedQty = '' === cell ? (line.expectedQty ?? null) : '-' === cell ? null : Number(cell) || 0;
      return {
        key: k,
        label: line.productLabel || k,
        status: 'matched',
        data: { poLineId: line.id, expectedQty },
        // Current expected drives the diff; null reads as empty so a first capture shows as an addition.
        existing: { expected_qty: null === line.expectedQty ? '' : String(line.expectedQty) },
      };
    });
  };
  ...
  const commitImport = (matched: ResolvedRow[]): void => {
    ...
    for (const m of matched) {
      const d = m.data as { poLineId: number; expectedQty: number | null } | undefined;
      if (!d) continue;
      setEdit(d.poLineId, d.expectedQty);
      staged++;
    }
    setImporting(false);
    if (staged > 0) {
      toast.success(sprintf(_n('%d expected quantity staged — review and Save.', '%d expected quantities staged — review and Save.', staged), staged));
    }
  };

  const save = createMutation(() => ({
    mutationFn: () =>
      api.put(`/procurement/purchase-orders/${props.poId}/expected`, {
        lines: editedLines(),
        documentType: docType(),
        reference: reference().trim() || null,
      }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['procurement', 'purchase-orders', String(props.poId)] });
    ...
    onError: (e: unknown) => toast.error(e instanceof Error ? e.message : __('Could not record the document.')),
  }));

  // ── DataGrid wiring ────────────────────────────────────────────────────────────────────────────
  const ch = createColumnHelper<PoLine>();
  const bespokeColumns = new Map<string, ColumnDef<PoLine, unknown>>([
    ['product', ch.display({ id: 'product', header: () => __('Product'), cell: (info) => <ProductCell line={info.row.original} /> }) as ColumnDef<PoLine, unknown>],
    ['sku', ch.display({ id: 'sku', header: () => __('SKU'), cell: (info) => <span class="block font-mono text-slate-500">{info.row.original.sku ?? '—'}</span> }) as ColumnDef<PoLine, unknown>],
    ['ordered', ch.display({ id: 'ordered', header: () => __('Ordered'), cell: (info) => <span class="block text-right tabular-nums">{info.row.original.requestedQty}</span> }) as ColumnDef<PoLine, unknown>],
    ['expected', ch.display({ id: 'expected', header: () => __('Expected'), cell: (info) => <ExpectedCell value={expectedFor(info.row.original)} ordered={info.row.original.requestedQty} /> }) as ColumnDef<PoLine, unknown>],
  ]);

  const columnMetas = (): GridColumnMeta[] => [
    colMeta({ id: 'product', label: __('Product'), description: __('Product on this PO line'), dataType: 'text', copyable: true, defaultWidth: 300 }),
    colMeta({ id: 'sku', label: __('SKU'), description: __('Our SKU for this product'), dataType: 'text', copyable: true, defaultWidth: 130 }),
    colMeta({ id: 'ordered', label: __('Ordered'), description: __('Quantity ordered — what Expected inherits (faded) when unset. “.” fills it into Expected.'), dataType: 'number', copyable: true, defaultWidth: 90 }),
    colMeta({ id: 'expected', label: __('Expected'), description: __('The supplier-confirmed quantity from the OA/ASN. “.” fills the ordered qty; blank clears it.'), dataType: 'number:receipt', kind: 'editable', editable: true, copyable: true, pasteable: true, defaultWidth: 120 }),
  ];
  ...
  const getStagedValue = (row: PoLine, columnId: string): StagedCell =>
    'expected' === columnId && undefined !== editFor(row.id) ? { staged: true, value: editFor(row.id) } : { staged: false, value: undefined };
  ...
  const resolveEditorMeta = (meta: GridColumnMeta, row: PoLine): GridColumnMeta =>
    'expected' === meta.id ? { ...meta, editorConfig: { dotDefault: row.requestedQty, ariaLabel: __('Expected') } } : meta;
  ...
  const [columnVisibility, setColumnVisibility] = persistedSignal<VisibilityState>('invflux:po-expected:colvis', {});
  const [columnOrder, setColumnOrder] = persistedSignal<ColumnOrderState>('invflux:po-expected:colorder', COLUMN_ORDER);
  const [columnSizing, setColumnSizing] = persistedSignal<Record<string, number>>('invflux:po-expected:colsize', {});
  const [expandedColumnSections, setExpandedColumnSections] = persistedSignal<string[]>('invflux:po-expected:colsections', ['document-capture']);
  const [rowSelection, setRowSelection] = createSignal<RowSelectionState>({});
  const [cellSelection, setCellSelection] = createSignal<SelectionState>(EMPTY_SELECTION);

  return (
    <div>
      <div class="mb-3 flex flex-wrap items-center gap-3 rounded border border-slate-200 bg-slate-50 p-2.5">
        <span class="text-sm font-medium text-slate-600">{__('Record supplier document')}</span>
        <SegmentedControl
          ariaLabel={__('Document type')}
          size="sm"
          options={[
            { value: 'oa', label: __('Order acknowledgement'), title: __('The supplier’s confirmation of the order') },
            { value: 'asn', label: __('Advance shipping notice'), title: __('The supplier’s dispatch notice') },
          ]}
          value={docType()}
          onChange={(v: DocType) => setDocType(v)}
        />
        <input
          type="text"
          value={reference()}
          onInput={(e) => setReference(e.currentTarget.value)}
          placeholder={__('Reference (optional)')}
          class="min-w-0 flex-1 rounded border border-slate-300 px-2 py-1 text-sm focus:border-primary focus:outline-none"
        />
      ...
      <div class="flex max-h-[70vh] flex-col pt-2">
        <DataGrid<PoLine>
          ...
          fallback={<div class="px-3 py-6 text-sm text-slate-500">{__('No lines on this purchase order.')}</div>}
        />
      </div>
    </div>
  );
...
/** Product label (+ GTIN sub-line for matching against the document). */
...
function ExpectedCell(props: { value: number | null; ordered: number }): JSX.Element {
  return (
    <Show when={null !== props.value} fallback={<span class="block text-right tabular-nums text-slate-300" title={__('Not yet confirmed — inherits the ordered quantity')}>{props.ordered}</span>}>
