...
import { Button, useHostNav } from '@invflux/ui';
import { DataGrid, type DataGridMenuItem, EMPTY_SELECTION, type GridColumnMeta, COMMON_ALIASES, fetchImportAliases, ImportWizard, type ImportField, learnImportAlias, type MappedRow, parseImportFile, type ResolvedRow, type SearchSelectOption, type SelectionState, type StagedCell, toast } from '@invflux/ui';
...
const COLUMN_ORDER = ['image', 'product', 'sku', 'supplier_sku', 'unit_price', 'moq', 'case_pack', 'lead_time'];
 ...
 */
export function SupplierProducts(props: { supplierId: number; supplierLeadTime: number | null; supplierCostDecimals: number; saveSlot?: HTMLElement }): JSX.Element {
  ...
  const key = (): [string, string, number, string] => ['procurement', 'suppliers', props.supplierId, 'products'];

  const query = createQuery(() => ({
    queryKey: key(),
    queryFn: () => api.get<SupplierProductsResponse>(`/procurement/suppliers/${props.supplierId}/products`),
  }));
  ...
  const stagedFor = (row: SupplierProduct, columnId: string): { staged: boolean; value: unknown } => {
  ...
  const sortKey = (p: SupplierProduct, columnId: string): string | number => {
    switch (columnId) {
      case 'product': return (p.productLabel ?? '').toLowerCase();
      case 'sku': return (p.sku ?? '￿').toLowerCase();
      case 'supplier_sku': return (p.supplierSku ?? '￿').toLowerCase();
      case 'unit_price': return null === p.unitPrice ? Number.POSITIVE_INFINITY : Number(p.unitPrice);
      case 'moq': return p.moq ?? defaults().moq ?? Number.POSITIVE_INFINITY;
      case 'case_pack': return p.casePack ?? defaults().casePack ?? Number.POSITIVE_INFINITY;
      case 'lead_time': return p.leadTimeDays ?? defaults().leadTime ?? Number.POSITIVE_INFINITY;
      default: return 0;
    }
  };
  const compareBy = (a: SupplierProduct, b: SupplierProduct, columnId: string): number => {
    ...
    return typeof ka === 'string' && typeof kb === 'string' ? ka.localeCompare(kb) : (ka as number) - (kb as number);
  };

  const visibleProducts = createMemo<SupplierProduct[]>(() => {
    const filtered = '' === filterText().trim()
      ? products()
      : products().filter((p) => fuzzyMatches(filterText(), [p.productLabel, p.sku, p.supplierSku, p.gtin]));
    const s = sorting()[0];
    const arr = [...filtered];
    if (undefined === s) {
      const idx = orderIdx();
  ...
  const draftLine = (): SupplierProduct => ({
    id: DRAFT_ID, supplierId: props.supplierId, subjectId: 0, postId: null, name: null, sku: null, supplierSku: null,
    gtin: null, imageUrl: null, productLabel: '', exists: true, unitPrice: null, currency: null,
    moq: null, casePack: null, leadTimeDays: null, priority: 0, createdAt: null, updatedAt: null,
  });
  ...
  const productSearch = async (q: string): Promise<SearchSelectOption[]> => {
    ...
    const inCatalogue = new Set(products().map((p) => p.postId).filter((id): id is number => null !== id));
    return res.products
      .filter((p) => !inCatalogue.has(p.postId))
      .map((p) => ({ value: String(p.postId), label: null === p.sku ? (p.name ?? `#${p.postId}`) : `${p.name ?? `#${p.postId}`} · ${p.sku}` }));
  };
  ...
  const addProduct = async (postId: number): Promise<void> => {
    try {
      const { product } = await api.post<{ product: SupplierProduct }>(`/procurement/suppliers/${props.supplierId}/products`, { woo_post_id: postId });
      await queryClient.invalidateQueries({ queryKey: key() });
      queueMicrotask(() => {
        const row = products().find((p) => p.id === product.id);
        const colId = undefined === row ? undefined : firstEditableColumnId(row, columnMetas(), canEdit, gridApi?.getSelectableColumnIds() ?? []);
  ...
  const columnMetas = createMemo<GridColumnMeta[]>(() => [
    ...
    colMeta({ id: 'product', label: __('Product'), dataType: PRODUCT_EDITOR_TYPE, sortable: true, copyable: true, defaultWidth: 280 }),
    colMeta({ id: 'sku', label: __('SKU'), dataType: 'text', sortable: true, copyable: true, defaultWidth: 120 }),
    colMeta({ id: 'supplier_sku', label: __('Supplier SKU'), dataType: 'text', kind: 'editable', editable: true, sortable: true, copyable: true, pasteable: true, defaultWidth: 150 }),
    colMeta({ id: 'unit_price', label: __('Unit price'), dataType: 'decimal:money', kind: 'editable', editable: true, sortable: true, copyable: true, pasteable: true, defaultWidth: 120 }),
    colMeta({ id: 'moq', label: __('MOQ'), dataType: 'number', kind: 'editable', editable: true, sortable: true, copyable: true, pasteable: true, defaultWidth: 80 }),
    colMeta({ id: 'case_pack', label: __('Case pack'), dataType: 'number', kind: 'editable', editable: true, sortable: true, copyable: true, pasteable: true, defaultWidth: 96 }),
    colMeta({ id: 'lead_time', label: __('Lead time'), dataType: 'number', kind: 'editable', editable: true, sortable: true, copyable: true, pasteable: true, defaultWidth: 96 }),
  ]);

  const ch = createColumnHelper<SupplierProduct>();
  const money = (v: unknown): string => (null === v || undefined === v || '' === v ? '—' : Number(v).toFixed(props.supplierCostDecimals));
  const bespokeColumns = new Map<string, ColumnDef<SupplierProduct, unknown>>([
    ['select', ch.display({ id: 'select',
      header: (info) => <input type="checkbox" class="cursor-pointer" checked={info.table.getIsAllRowsSelected()} title={__('Select all')} onChange={info.table.getToggleAllRowsSelectedHandler()} />,
      cell: (info) => <Show when={!isDraft(info.row.original)}><input type="checkbox" class="cursor-pointer" checked={info.row.getIsSelected()} onChange={info.row.getToggleSelectedHandler()} onClick={(e) => e.stopPropagation()} /></Show>,
    ...
    ['product', ch.accessor((r) => r.productLabel, { id: 'product', enableSorting: true, header: () => __('Product'), cell: (info) => (
      <Show when={!isDraft(info.row.original)} fallback={<span class="italic text-text-muted">{__('＊ Add a product ＊')}</span>}>
        <ProductCell link={info.row.original} />
      </Show>
    ) }) as ColumnDef<SupplierProduct, unknown>],
    ['sku', ch.accessor((r) => r.sku, { id: 'sku', enableSorting: true, header: () => __('SKU'), cell: (info) => <Show when={!isDraft(info.row.original) && info.row.original.sku} fallback={<Show when={!isDraft(info.row.original)}><span class="text-slate-300">—</span></Show>}>{(s) => <span class="block font-mono text-slate-500">{s()}</span>}</Show> }) as ColumnDef<SupplierProduct, unknown>],
    ['supplier_sku', ch.accessor((r) => r.supplierSku, { id: 'supplier_sku', enableSorting: true, header: () => __('Supplier SKU'), cell: (info) => {
      ...
      const getS = (): { staged: boolean; value: unknown } => stagedFor(info.row.original, 'supplier_sku');
      const value = (): string | null => (getS().staged ? getS().value : info.row.original.supplierSku) as string | null;
      return <Show when={null !== value() && '' !== value()} fallback={<span class="text-slate-300">—</span>}><span classList={{ 'text-green-700': getS().staged, 'text-slate-700': !getS().staged }}>{value()}</span></Show>;
    } }) as ColumnDef<SupplierProduct, unknown>],
    ['unit_price', ch.accessor((r) => r.unitPrice, { id: 'unit_price', enableSorting: true, header: () => __('Unit price'), cell: (info) => {
      if (isDraft(info.row.original)) return null;
      // Use a lazy accessor so reads of dirty() happen inside JSX effects (reactive in SolidJS).
      const getS = (): { staged: boolean; value: unknown } => stagedFor(info.row.original, 'unit_price');
      const value = (): unknown => getS().staged ? getS().value : info.row.original.unitPrice;
      return <span class="block text-right tabular-nums" classList={{ 'text-green-700': getS().staged, 'font-semibold text-slate-800': !getS().staged && null !== info.row.original.unitPrice, 'text-slate-300': !getS().staged && null === info.row.original.unitPrice }}>{money(value())}</span>;
    } }) as ColumnDef<SupplierProduct, unknown>],
    ['moq', ch.accessor((r) => r.moq, { id: 'moq', enableSorting: true, header: () => __('MOQ'), cell: (info) => <Show when={!isDraft(info.row.original)}><TermCell s={() => stagedFor(info.row.original, 'moq')} value={info.row.original.moq} fallback={defaults().moq} /></Show> }) as ColumnDef<SupplierProduct, unknown>],
    ['case_pack', ch.accessor((r) => r.casePack, { id: 'case_pack', enableSorting: true, header: () => __('Case pack'), cell: (info) => <Show when={!isDraft(info.row.original)}><TermCell s={() => stagedFor(info.row.original, 'case_pack')} value={info.row.original.casePack} fallback={defaults().casePack} /></Show> }) as ColumnDef<SupplierProduct, unknown>],
    ['lead_time', ch.accessor((r) => r.leadTimeDays, { id: 'lead_time', enableSorting: true, header: () => __('Lead time'), cell: (info) => <Show when={!isDraft(info.row.original)}><TermCell s={() => stagedFor(info.row.original, 'lead_time')} value={info.row.original.leadTimeDays} fallback={defaults().leadTime} suffix=" d" /></Show> }) as ColumnDef<SupplierProduct, unknown>],
  ]);

  // ── DataGrid state ──────────────────────────────────────────────────────────────────────────────
  const [columnVisibility, setColumnVisibility] = persistedSignal<VisibilityState>('invflux:supplier-products:colvis', {});
  const [columnOrder, setColumnOrder] = persistedSignal<ColumnOrderState>('invflux:supplier-products:colorder', COLUMN_ORDER);
  const [columnSizing, setColumnSizing] = persistedSignal<Record<string, number>>('invflux:supplier-products:colsize', {});
  const [expandedColumnSections, setExpandedColumnSections] = persistedSignal<string[]>('invflux:supplier-products:colsections', ['supplier-products']);
  ...
  const getValue = (row: SupplierProduct, columnId: string): unknown => {
    ...
    switch (columnId) {
      ...
      case 'unit_price': return row.unitPrice;
      case 'moq': return row.moq;
      case 'case_pack': return row.casePack;
      case 'lead_time': return row.leadTimeDays;
  ...
  const resolveClearedValue = (_row: SupplierProduct, meta: GridColumnMeta): { ok: boolean; value: unknown } =>
  ...
  const resolveEditorMeta = (meta: GridColumnMeta, row: SupplierProduct): GridColumnMeta =>
    isDraft(row) && 'product' === meta.id ? { ...meta, editorConfig: { search: productSearch, portalRoot } } : meta;
  ...
  const stageEdit = (row: SupplierProduct, columnId: string, next: unknown): void => {
    ...
    const value: string | number | null =
      blank ? null : 'supplier_sku' === columnId || 'unit_price' === columnId ? String(next) : Number(next);
    ...
    const rawPersisted: unknown = ({ supplier_sku: row.supplierSku, unit_price: row.unitPrice, moq: row.moq, case_pack: row.casePack, lead_time: row.leadTimeDays } as Record<string, unknown>)[columnId];
    const persistedCoerced: string | number | null =
      null === rawPersisted || undefined === rawPersisted ? null
      : 'supplier_sku' === columnId || 'unit_price' === columnId ? String(rawPersisted) : Number(rawPersisted);
  ...
  const onStageEdit = (row: SupplierProduct, columnId: string, _original: unknown, next: unknown): void => {
  ...
  const onCommitNavigate = (row: SupplierProduct, columnId: string): boolean => isDraft(row) && 'product' === columnId;
      ...
      await Promise.all(
        Object.keys(d).map((idStr) => {
          ...
          return api.patch(`/procurement/suppliers/${props.supplierId}/products/${id}`, {
            ...
            lead_time_days: 'lead_time_days' in patch ? patch.lead_time_days : (link.leadTimeDays ?? null),
          });
        }),
      );
    },
  ...
  const delMany = createMutation(() => ({
    mutationFn: (ids: number[]) => Promise.all(ids.map((id) => api.del(`/procurement/suppliers/${props.supplierId}/products/${id}`))),
  ...
  const selectedIds = (): number[] => Object.entries(rowSelection()).filter(([, v]) => v).map(([k]) => Number(k)).filter((id) => DRAFT_ID !== id);
  const createPo = createMutation(() => ({
    mutationFn: () => {
      const chosen = new Set(selectedIds());
      const lines = products().filter((p) => chosen.has(p.id)).map((p) => ({ subject_id: p.subjectId, requested_qty: 0, unit_cost: p.unitPrice }));
      return api.post<{ purchaseOrder: { id: number } }>('/procurement/purchase-orders', { supplier_id: props.supplierId, lines });
    },
    onSuccess: (data) => {
  ...
  const aliasesQuery = createQuery(() => ({ queryKey: ['procurement', 'import-aliases'], queryFn: () => fetchImportAliases(ctx), staleTime: 5 * 60 * 1000 }));
  const learnAlias = (concept: string, header: string): void => {
    void learnImportAlias(ctx, concept, header)
      ...
      .catch((e: unknown) => toast.error(e instanceof Error ? e.message : __('Could not save the alias.')));
  };
  const fmtPrice = (v: string): string => ('' === v || '-' === v || Number.isNaN(Number(v)) ? v : Number(v).toFixed(props.supplierCostDecimals));
  const importFields: ImportField[] = [
    ...
    { key: 'sku', label: __('SKU'), aliases: [...COMMON_ALIASES.sku], keyCandidate: true, keyPriority: 1, keyOnly: true },
    { key: 'supplier_sku', label: __('Supplier SKU'), aliases: [...COMMON_ALIASES.supplier_sku], keyCandidate: true, keyPriority: 2, keyOnly: !isPro },
    { key: 'gtin', label: __('Barcode (GTIN/EAN)'), aliases: [...COMMON_ALIASES.gtin], keyCandidate: true, keyPriority: 3, keyOnly: true },
    { key: 'unit_price', label: __('Unit price'), aliasKey: 'cost', aliases: [...COMMON_ALIASES.cost], numeric: true, format: fmtPrice },
    { key: 'moq', label: __('MOQ'), aliases: ['min order', 'minimum order', 'min qty', 'minimum', 'quantité minimum', 'minimum de commande'], numeric: true },
    { key: 'case_pack', label: __('Case pack'), aliases: ['pack', 'pack size', 'carton', 'colisage', 'conditionnement', 'unités par carton'], numeric: true },
    { key: 'lead_time_days', label: __('Lead time (days)'), aliases: ['lead time', 'leadtime', 'delay', 'délai', 'délai livraison', 'delai'], numeric: true },
  ];
  ...
  const resolveImport = async (rows: MappedRow[], keyField: string): Promise<ResolvedRow[]> => {
    ...
    const { resolved } = await api.post<{
      resolved: Record<string, { postId: number | null; isVariation: boolean; name: string; subjectId: number | null; supplierSku?: string | null }>;
    ...
    return rows.map((r): ResolvedRow => {
      ...
      const data: Record<string, unknown> = null !== hit.postId ? { woo_post_id: hit.postId, is_variation: hit.isVariation } : { subject_id: hit.subjectId };
      ...
      num('case_pack');
      num('lead_time_days');
      const supSku = 'supplier_sku' in r && 'supplier_sku' !== keyField ? (r.supplier_sku ?? '').trim() : undefined;
      ...
      if (link) {
        ...
        if ('unit_price' in r) existing.unit_price = null === link.unitPrice ? '' : String(link.unitPrice);
        if ('moq' in r) existing.moq = null === link.moq ? '' : String(link.moq);
        if ('case_pack' in r) existing.case_pack = null === link.casePack ? '' : String(link.casePack);
        if ('lead_time_days' in r) existing.lead_time_days = null === link.leadTimeDays ? '' : String(link.leadTimeDays);
        // Current scoped supplier SKU, so re-importing the same code reads unchanged (not a green add).
        if ('supplier_sku' in r && 'supplier_sku' !== keyField) existing.supplier_sku = hit.supplierSku ?? '';
  ...
  const saveButton = (): JSX.Element => (
    <Button
      disabled={0 === dirtyCount() || save.isPending}
      onClick={() => save.mutate()}
    >
      <Show when={dirtyCount() > 0} fallback={__('Save')}>{sprintf(__('Save %d change(s)'), dirtyCount())}</Show>
    </Button>
  );
  ...
  return (
    <section>
      ...
      <Show when={query.isError}>
        <p class="text-red-700">{__('Failed to load products.')}</p>
      </Show>
      ...
      <Show when={query.isSuccess}>
        ...
        <div class="mb-2 flex items-center gap-3">
          ...
          <div class="flex-1" />
          <a href={workbenchUrl()} target="_blank" rel="noopener" class="text-sm text-primary hover:underline">
        ...
        <div class="flex max-h-[70vh] flex-col">
          <DataGrid<SupplierProduct>
            ...
            fallback={<div class="px-3 py-6 text-sm text-slate-500">{__('No products in this catalogue yet.')}</div>}
          />
        </div>
      </Show>
    </section>
...
function ProductCell(props: { link: SupplierProduct }): JSX.Element {
  return <div class="font-medium" classList={{ 'text-text-muted italic': !props.link.exists }}>{props.link.productLabel}</div>;
}

/** A numeric term cell: staged (green), an explicit override (bold), or the inherited default (light). */
function TermCell(props: { s: () => { staged: boolean; value: unknown }; value: number | null; fallback: number | null; suffix?: string }): JSX.Element {
  const raw = (): number | null => (props.s().staged ? (props.s().value as number | null) : props.value);
  const effective = (): number | null => raw() ?? props.fallback;
  return (
    <Show when={null !== effective()} fallback={<span class="block text-right text-slate-300">—</span>}>
      <span
