...
import { Button, SearchSelect, toast } from '@invflux/ui';
...
import { countryName, countryOptions, currencyOptions, stateName, stateOptions } from '../../lib/geo';
...
const TAX_TREATMENTS: Array<{ value: string; label: () => string; hint: () => string }> = [
  {
    value: 'standard',
    label: () => __('Standard'),
    hint: () => __('Ordinary domestic purchasing — the supplier charges tax and the order shows net, tax and gross.'),
  },
  {
    value: 'reverse_charge',
    label: () => __('Reverse charge'),
    hint: () => __('Cross-border business-to-business: the supplier invoices at 0% and you account for the tax yourself. Both tax numbers must appear on the order.'),
  },
  {
    value: 'export_exempt',
    label: () => __('Import / export exempt'),
    hint: () => __('Bought from outside your tax area — nothing is charged here; any tax arises at customs.'),
  },
  { value: 'exempt', label: () => __('Exempt'), hint: () => __('The goods or the supply are exempt in your jurisdiction — 0%, with the exemption stated on the order.') },
  {
    value: 'not_registered',
    label: () => __('Supplier not registered'),
    hint: () => __('The supplier is below the registration threshold and charges no tax at all — the order carries no tax block.'),
  },
];
...
const taxTreatmentLabel = (value: string): string => TAX_TREATMENTS.find((t) => t.value === value)?.label() ?? value;
...
export function SupplierDetail(props: { id: string }): JSX.Element {
  ...
  const replenish = createMutation(() => ({
    mutationFn: () => api.post<{ purchaseOrder: { id: number } | null }>('/procurement/purchase-orders/replenish', { supplier_id: Number(props.id) }),
    onSuccess: (data) => {
    ...
    onError: (e: unknown) => toast.error(e instanceof Error ? e.message : __('Could not create the replenishment draft.')),
  }));
  ...
  return (
    <section>
      ...
      <Show when={query.isError}>
        <p class="mt-4 text-red-700">{__('Failed to load supplier.')}</p>
      </Show>
      ...
      <Show when={query.data?.supplier}>
        {(s) => (
          <>
            <header class="mb-4 flex flex-wrap items-center gap-3">
              ...
              <div class="ml-auto flex items-center gap-3">
                <Button
                  ...
                  title={__('Create a draft PO for this supplier’s products at or below their reorder threshold')}
                >
            ...
            <nav class="mb-4 flex gap-5 border-b border-slate-200 text-sm">
              <For each={TABS}>
                {(t) => (
                  <A
                    href={'details' === t.key ? `/suppliers/${props.id}` : `/suppliers/${props.id}/${t.key}`}
                    class="-mb-px border-b-2 pb-2"
                    classList={{
                      'border-primary text-primary font-medium': tab() === t.key,
            ...
            <Switch>
              ...
              <Match when={'products' === tab()}>
                <SupplierProducts supplierId={Number(props.id)} supplierLeadTime={s().leadTimeDays} supplierCostDecimals={s().costDecimals} saveSlot={saveSlot()} />
              </Match>
              <Match when={true}>
...
function DetailsTab(props: { supplier: Supplier }): JSX.Element {
  ...
  return (
    <Show when={editing()} fallback={<ReadView supplier={props.supplier} onEdit={() => setEditing(true)} />}>
...
function ReadView(props: { supplier: Supplier; onEdit: () => void }): JSX.Element {
  ...
  const dash = (v: string | number | null): JSX.Element => (null === v || '' === v ? <span class="text-text-muted">—</span> : <>{v}</>);
  ...
  return (
    <div>
      <dl class="grid max-w-lg grid-cols-[10rem_1fr] gap-y-2 text-sm">
        ...
        <Row label={__('Lead time')}>{null === s().leadTimeDays ? dash(null) : <>{s().leadTimeDays} d</>}</Row>
        ...
        <Row label={__('Purchase tax rate')}>
          <Show when={CHARGES_TAX.includes(s().taxTreatment)} fallback={<span class="text-text-muted">{__('n/a under this regime')}</span>}>
        ...
        <Row label={__('Terms & conditions')}>
          <Show when={s().termsConditions} fallback={<span class="text-text-muted">{__('Store default')}</span>}>
            {(t) => <span class="whitespace-pre-line">{t()}</span>}
          </Show>
        </Row>
        <Row label={__('Document language')}>
          <Show when={documentLocaleLabel(ctx, s().documentLanguage)} fallback={<span class="text-text-muted">{__('Store default')}</span>}>
            {(label) => <>{label()}</>}
          </Show>
        </Row>
        <Row label={__('Status')}>
...
function EditForm(props: { supplier: Supplier; onDone: () => void }): JSX.Element {
  ...
  const [leadTime, setLeadTime] = createSignal(null === s.leadTimeDays ? '' : String(s.leadTimeDays));
  ...
  const [termsConditions, setTermsConditions] = createSignal(s.termsConditions ?? '');
  ...
  const installLanguage = async (locale: string): Promise<void> => {
    ...
    try {
      const r = await api.post<InstallLanguageResponse>(`/procurement/document-languages/${locale}`, {});
      ...
      toast.success(sprintf(__('%s added. Purchase orders can now be written in it.'), r.available[r.installed] ?? locale));
    } catch (e: unknown) {
  ...
  const taxTreatmentHint = (): string => TAX_TREATMENTS.find((t) => t.value === taxTreatment())?.hint() ?? '';
    ...
    <form class="max-w-lg" onSubmit={submit}>
      <div class="grid grid-cols-2 gap-3">
        ...
        <label class={FIELD}>
          ...
          <input class={INPUT} value={nickname()} onInput={(e) => setNickname(e.currentTarget.value)} />
        </label>
        <label class={FIELD}>
          {__('Code')}{' '}
          <Hint text={__('Your internal reference for this supplier — e.g. an accounting or ERP supplier number. Free-form; not a URL slug, and not shown to the supplier.')} />
        ...
        <label class={FIELD}>
          ...
          />
          <input class={INPUT} value={taxNumber()} onInput={(e) => setTaxNumber(e.currentTarget.value)} />
        </label>
      </div>
      ...
      <div class="mt-3 grid grid-cols-3 gap-3">
        ...
        <label class={FIELD}>
          {__('Lead time (days)')}
          <input class={INPUT} type="number" min="0" value={leadTime()} onInput={(e) => setLeadTime(e.currentTarget.value)} />
        </label>
        <label class={FIELD}>
          {__('Cost decimals')}
          <select class={INPUT} value={costDecimals()} onChange={(e) => setCostDecimals(e.currentTarget.value)}>
      ...
      <label class={`${FIELD} mt-3`}>
        {__('Payment terms')}
        <input class={INPUT} value={paymentTerms()} onInput={(e) => setPaymentTerms(e.currentTarget.value)} />
      </label>
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <label class={FIELD}>
          {__('Ordering URL')}
          <input class={INPUT} type="url" value={orderingUrl()} onInput={(e) => setOrderingUrl(e.currentTarget.value)} />
        </label>
      </div>
      ...
      <label class={FIELD}>
        {__('Address line 1')}
        <input class={INPUT} value={address1()} onInput={(e) => setAddress1(e.currentTarget.value)} />
      </label>
      <label class={`${FIELD} mt-3`}>
        {__('Address line 2')}
        <input class={INPUT} value={address2()} onInput={(e) => setAddress2(e.currentTarget.value)} />
      </label>
      <div class="mt-3 grid grid-cols-2 gap-3">
        ...
        <label class={FIELD}>
          {__('Postcode')}
          <input class={INPUT} value={postcode()} onInput={(e) => setPostcode(e.currentTarget.value)} />
        </label>
        <div class={FIELD}>
          {__('Country')}
          <div class="mt-1">
        ...
        <div class={FIELD}>
          ...
          <div class="mt-1">
            <Show
              ...
              fallback={<input class={INPUT} value={state()} onInput={(e) => setState(e.currentTarget.value)} />}
            >
              <SearchSelect
      ...
      <label class={FIELD}>
        {__('Our account no.')}{' '}
        <Hint text={__('The customer reference the supplier holds for you — what their accounts department quotes. The mirror of “Code”, which is your reference for them.')} />
        <input class={INPUT} value={accountNumber()} onInput={(e) => setAccountNumber(e.currentTarget.value)} />
      </label>
      <div class="mt-3 grid grid-cols-2 gap-3">
        <div class={FIELD}>
          {__('Tax treatment')}{' '}
          <Hint text={taxTreatmentHint()} />
          <select class={INPUT} value={taxTreatment()} onChange={(e) => setTaxTreatment(e.currentTarget.value)}>
      ...
      <label class="mt-3 flex items-start gap-2 text-sm text-slate-700">
        <input type="checkbox" class="mt-0.5" checked={quotesIncludeTax()} onChange={(e) => setQuotesIncludeTax(e.currentTarget.checked)} />
        <span>
          {__('This supplier quotes tax-inclusive prices')}{' '}
          <Hint text={__('Only changes how you type a cost in: a gross price is converted down on entry, so what is stored — and what every cost figure means — stays net.')} />
        </span>
      </label>
      <label class={`${FIELD} mt-3`}>
        {__('Terms & conditions')}{' '}
        <Hint text={__('Printed on purchase orders for this supplier. Leave empty to use the store-wide terms; a single order can still override this.')} />
        <textarea class={INPUT} rows="3" value={termsConditions()} onInput={(e) => setTermsConditions(e.currentTarget.value)} />
      </label>
      <label class={`${FIELD} mt-3`}>
        ...
        />
        <select class={INPUT} value={documentLanguage()} onChange={(e) => setDocumentLanguage(e.currentTarget.value)}>
          ...
          <For each={asOptions(availableLocales())}>{(l) => <option value={l.value}>{l.label}</option>}</For>
        </select>
      </label>
            ...
            {(l, i) => (
              <>
                ...
                <button
                  ...
                  title={sprintf(__('Add %s to WordPress so purchase orders can be written in it'), l.label)}
